...
public class MultiJsonRasAnonymityReader extends JsonAnonymityReader<MultiRasAnonymity> {
    protected final Set<Dimension> dimensionSet;

    public MultiJsonRasAnonymityReader(JSONObject jsonObject, JsonFileReader jsonFileReader, Set<Dimension> dimensionSet) {
        super(jsonObject, jsonFileReader);
        this.dimensionSet = dimensionSet;
    }

    public MultiRasAnonymity readAnonymityConfig(String key) throws ReaderException, InvalidConfigException {
        Map<Dimension, Integer> anonymityMap = new HashMap<>();
        Map<Dimension, Integer> replacingAnonymityMap = new HashMap<>();
        ...
        for (JSONObject anonymityJsonObject : anonymityJsonObjects) {
            ...
            replacingAnonymityMap.putAll(addingAnonymityPair);
        }
        return new MultiRasAnonymity(anonymityMap, replacingAnonymityMap);
    }
