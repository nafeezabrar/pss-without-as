...
import pss.conversion.data_to_result.SimplestAdversaryResultToResultConvertable;
...
public class SimplestAdversaryResultToTabularResultConverter implements SimplestAdversaryResultToResultConvertable {
    ...
    private SingleTableResult generateTableResult(PssType pssType, OoiUserGroupMappable ooiUserGroupMapper, List<FullCycleResult> fullCycleResults, List<ObservedReport> observedReports, AdversaryResultWithSingleOcTable adversaryResult, String resultDirectory) throws PssException, IOException, ClassNotFoundException {
        ...
        addRows(tableResult, pssType, ooiUserGroupMapper, fullCycleResults, observedReports, adversaryResult, resultDirectory);
        return tableResult;
    }
    ...
    private void addRows(SingleTableResult tableResult, PssType pssType, OoiUserGroupMappable ooiUserGroupMapper, List<FullCycleResult> fullCycleResults, List<ObservedReport> observedReports, AdversaryResultWithSingleOcTable adversaryResult, String resultDirectory) throws PssException, IOException, ClassNotFoundException {
        ...
        List<SingleOcTable> ocTables = objectReader.getObjects(totalReportCount);
        for (int i = 0; i < totalReportCount; i++) {
            ...
            OcTable adversaryOcTable = ocTables.get(i);
            TabularResultRow resultRow = generateRow(anonymizationResult, deanonymizationResult, observedReports.get(i), ooiUserGroupMapper, pssType, adversaryOcTable);
            tableResult.addRow(resultRow);
        }

    }

    private TabularResultRow generateRow(AnonymizationResult anonymizationResult, DeanonymizationResult deanonymizationResult, ObservedReport observedReport, OoiUserGroupMappable ooiUserGroupMapper, PssType pssType, OcTable adversaryOcTable) throws PssException {
