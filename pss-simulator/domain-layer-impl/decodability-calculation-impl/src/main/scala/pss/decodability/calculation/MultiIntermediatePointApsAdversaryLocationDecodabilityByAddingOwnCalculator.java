...
public class MultiIntermediatePointApsAdversaryLocationDecodabilityByAddingOwnCalculator implements AdversaryIntermediatePointsDecodabilityCalculable<LocationDecodabilityResult, AdversaryResultWithSingleOcTable> {
    ...
    private Map<FinalReport, MultiOcTable> ocTableMap;


    public MultiIntermediatePointApsAdversaryLocationDecodabilityByAddingOwnCalculator(ApsAdversary apsAdversary, ValueMap valueMap, PssVariables pssVariables, PssType pssType, Set<PssGroup> pssGroups, OoiUserGroupMappable ooiUserGroupMapper, List<ObservedReport> observedReports, List<FinalReport> finalReports, String resultDirectory, OcTableReadable ocTableReader) throws PssException {
    ...
    private void initReportedByAdversaryMap() throws PssException {
        reportedLocationsByAdversary = new HashMap<>();
        for (ObservedReport observedReport : observedReports) {
            int reportedUserId = observedReport.getReportData().getUserId();
            Value reportedValue = observedReport.getValue();

            if (adversaryUserIds.contains(reportedUserId)) {
                LocationOoi locationOoi = getLocationOoi(observedReport);
                reportedLocationsByAdversary.put(reportedValue, locationOoi);
            }
    ...
    public LocationDecodabilityResult calculateDecodability(AdversaryResultWithSingleOcTable adversaryResult, FinalReport finalReport) throws PssException, IOException, ClassNotFoundException {
        ...
        int totalDecoded = reportedLocationsByAdversary.size();
...
//            TextFileOcTableReaderWithIndex<MultiOcTable> objectReader = new TextFileOcTableReaderWithIndex(ocTableFileName);
////            FileInputStream fileInputStream = new FileInputStream(ocTableFileName);
////            ObjectInputStream objectInputStream = new ObjectInputStream(fileInputStream);
////            MultiOcTable ocTable = (MultiOcTable) objectInputStream.readObject();
