...
import pss.conversion.data_to_result.FullCycleResultToResultConvertable;
...
public class FullCycleResultToTabularResultConverter implements FullCycleResultToResultConvertable {
    ...
    @Override
    public TabularResult generateFullCycleResult(PssType pssType, OoiUserGroupMappable ooiUserGroupMapper, List<FullCycleResult> fullCycleResults, List<ObservedReport> observedReports) throws PssException {
        TabularResult tabularResult = new TabularResult(getAnonymizationResultTableHeading());
        SingleTableResult singleTableResult = generateTableResult(pssType, ooiUserGroupMapper, fullCycleResults, observedReports);
        tabularResult.addTableResult(singleTableResult);
        return tabularResult;
    }

    private SingleTableResult generateTableResult(PssType pssType, OoiUserGroupMappable ooiUserGroupMapper, List<FullCycleResult> fullCycleResults, List<ObservedReport> observedReports) throws PssException {
        SingleTableResult tableResult = new SingleTableResult(getAnonymizationResultTableHeading());
        initColumns(pssType, fullCycleResults);
        addColumns(tableResult);
        addRows(tableResult, pssType, ooiUserGroupMapper, fullCycleResults, observedReports);
        return tableResult;
    }

    private void initColumns(PssType pssType, List<FullCycleResult> fullCycleResults) throws PssException {
    ...
    private void addRows(SingleTableResult tableResult, PssType pssType, OoiUserGroupMappable ooiUserGroupMapper, List<FullCycleResult> fullCycleResults, List<ObservedReport> observedReports) throws PssException {
        ...
        for (int i = 0; i < totalReportCount; i++) {
            ...
            TabularResultRow resultRow = generateRow(anonymizationResult, deanonymizationResult, observedReports.get(i), ooiUserGroupMapper, pssType);
            tableResult.addRow(resultRow);
        }

    }

    private TabularResultRow generateRow(AnonymizationResult anonymizationResult, DeanonymizationResult deanonymizationResult, ObservedReport observedReport, OoiUserGroupMappable ooiUserGroupMapper, PssType pssType) throws PssException {
